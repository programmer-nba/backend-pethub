...
const {
  ReturnProductShall,
} = require("../../models/product/return.product.shell.model.js");
  ...
  try {
    ...
    if (!update_typemember) {
      return res
        ...
        .send({
          status: false,
          message: "ไม่พบสมาชิกที่ตรงกับหมายเลขโทรศัพท์ที่ระบุ",
        });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "อัปเดตประเภทลูกค้าสำเร็จ" });
  } catch (error) {
    console.log(error);
...
exports.getDetailsProduct = async (req, res) => {
  try {
    const id = req.params.id;
    const preorder_list = await PreOrderProductShell.findOne({
      ordernumbershell: id,
    });
    if (preorder_list) {
...
exports.UpdateProductAmount = async (req, res) => {
  try {
    ...
    const productCount = await PreOrderProductShell.findOne({
      ordernumbershell: ordernumbershell,
    });
    if (!productCount) {
      return res
        .status(400)
        .send({ message: "กรุณากรอก ordernumber ก่อน", status: false });
    }
    ...
    for (let product of productDetails) {
      ...
      if (result.nModified === 0) {
        return res.status(404).send({
          message:
            "ไม่พบข้อมูลสินค้าที่ต้องการแก้ไขหรือข้อมูลสินค้านั้นอาจจะไม่ถูกอัปเดต",
          status: false,
        });
      }
    }
...
exports.ProductReturn = async (req, res) => {
  try {
    ...
    const preorder = await PreOrderProductShell.findOne({
      ordernumbershell: ordernumbershell,
    });
    if (!preorder) {
      return res.status(500).send({
    ...
    const findreturn = productdetail.filter(
      (item) => item.product_id == req.body.product_detail[0].product_id
    );
    ...
    const dataedit = {
      ...
      product_name: findreturn[0].product_name,
      product_amount:
        findreturn[0].product_amount -
        req.body.product_detail[0].product_amount,
      product_logo: findreturn[0].product_logo,
      barcode: findreturn[0].barcode,
      timestamps: findreturn[0].timestamps,
    };
    const filerpreorder = preorder.product_detail.filter(
      (item) => item.product_id != req.body.product_detail[0].product_id
    );
    if (
      findreturn[0].product_amount - req.body.product_detail[0].product_amount >
      0
    ) {
...
exports.fildAllProducShalltReturn = async (req, res) => {
  try {
    ...
    if (employee) {
      ...
      return res
        .status(404)
        .send({ message: "ไม่พบรายการส่งคืนสินค้า", status: false });
    }
  } catch (err) {
    res.status(500).send({ message: "มีบางอย่างผิดพลาด", status: false });
  }
